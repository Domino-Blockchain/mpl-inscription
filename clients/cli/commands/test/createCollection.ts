...
export async function test_createCollection(
  rpc: string,
  keypair: string,
  assetsDir: string,
  concurrency: number
  ...
  umi.use(
    nftStorageUploader({
      token:
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweDRGRGRGMDk0Mjg3OWE3ZjQ0OTU0M2RERGUzRDdDY0E3N2Q0MzY1ZDYiLCJpc3MiOiJuZnQtc3RvcmFnZSIsImlhdCI6MTcwNDkxNTc5NTQ3NSwibmFtZSI6Ikluc2NyaXB0aW9ucyJ9.iRNSAtmzOm8RKW8B-Ly0G99VTdQkNnh4pDEIxfnW5v0",
    })
  );
  ...
  await pMap(
    ...
    async (file) => {
      ...
      const metadata = JSON.parse(
        readFileSync(`${assetsDir}/${mint}.metadata`).toString()
      );
      await createAndMint(umi, {
        mint: newMint,
        name: metadata.name,
